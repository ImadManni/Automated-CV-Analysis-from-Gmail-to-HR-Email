...
import { demoInterviews } from '@/mock/demoData'
...
export function InterviewsPage() {
  ...
  const loadInterviews = async () => {
    ...
    try {
      ...
      setInterviews(USE_DEMO_FALLBACK ? demoInterviews : [])
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className={styles.container}>
        ...
        <div className={styles.toolbar}>
          <p className={styles.subtitle}>
            Liste des entretiens avec détails et actions (même couleurs et boutons que la fiche candidature).
          </p>
        ...
        {loading ? (
          <p className={styles.info}>Chargement des entretiens…</p>
        ) : interviews.length === 0 ? (
              ...
              <div key={iv.id} className={styles.card}>
                <div className={styles.cardMain}>
                  <p className={styles.cardTitle}>
                    {iv.candidateName || 'Candidat'} — {iv.subject || 'Sans objet'}
                  </p>
                  <p className={styles.cardMeta}>
                    {formatDate(iv.scheduledAt)} · {iv.mode} · {iv.location}
                  </p>
                  <p className={styles.cardEmail}>{iv.email || '—'}</p>
