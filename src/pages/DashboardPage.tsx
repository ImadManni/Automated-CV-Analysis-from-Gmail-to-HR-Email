...
export function DashboardPage() {
  ...
  const normalizeSearch = useCallback((v: string) => {
    return String(v || '')
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
  ...
  const matchesCandidateSearch = useCallback(
    (c: (typeof items)[number], rawQuery: string) => {
      const q = normalizeSearch(rawQuery)
      if (!q) return true
      const decision = normalizeSearch(c.decision || '')
      const decisionAliases =
        decision.includes('accep')
          ? ['acceptee', 'acceptee', 'accepted', 'accept']
          : decision.includes('refus')
            ? ['refusee', 'refusee', 'refused', 'reject']
            : decision.includes('revoir')
              ? ['a revoir', 'arevoir', 'to review', 'review']
              : decision.includes('non_lisible') || decision.includes('non lisible')
                ? ['non lisible', 'non_lisible', 'unreadable', 'illisible']
                : []
  ...
  const filteredMatrixItems = useMemo(
    () => items.filter((c) => matchesCandidateSearch(c, matrixSearch)),
    [items, matchesCandidateSearch, matrixSearch],
  )
  const filteredHiringItems = useMemo(
    () => items.filter((c) => matchesCandidateSearch(c, hiringSearch)),
    [items, matchesCandidateSearch, hiringSearch],
  )

  const filteredMatrixInterviews = useMemo(() => {
    const ids = new Set(filteredMatrixItems.map((c) => String(c.id)))
    return interviews.filter((iv) => ids.has(String(iv.candidatureId)))
  }, [filteredMatrixItems, interviews])
        ...
        <section className={styles.section}>
          ...
          <div className={styles.searchRow}>
            <input
              ...
              placeholder="Search candidat (nom, email, objet, BU...)"
              className={styles.searchInput}
            />
            <span className={styles.searchCount}>
              {filteredMatrixItems.length} / {items.length}
            </span>
          </div>
          <InterviewMatrixTable
            candidatures={filteredMatrixItems}
            interviews={filteredMatrixInterviews}
        ...
        <section className={styles.section}>
          ...
          <div className={styles.searchRow}>
            <input
              ...
              placeholder="Search candidature (nom, email, objet, BU...)"
              className={styles.searchInput}
            />
            <span className={styles.searchCount}>
              {filteredHiringItems.length} / {items.length}
            </span>
          </div>
          <HiringDelaysTable items={filteredHiringItems} interviews={interviews} embedded />
        </section>
      </div>
    </div>
  )
