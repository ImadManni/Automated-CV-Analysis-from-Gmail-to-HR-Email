...
const navLinks: { to: string; label: string; end?: boolean; icon?: typeof HiChartBar; authOnly?: boolean }[] = [
  { to: '/dashboard', label: 'Dashboard', end: true, icon: HiChartBar, authOnly: true },
  { to: '/entretiens', label: 'Entretiens', end: true, authOnly: true },
  { to: '/campaigns', label: 'Campagnes', end: false, authOnly: true },
]
    ...
    <HeaderWrapper {...headerProps}>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            {(token ? navLinks : navLinks.filter((l) => !l.authOnly)).map((link) => {
